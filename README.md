...
> **公开范围说明**
>
> 公开项目展示仓库，主要提供项目介绍、截图和演示材料。完整源码已在独立的
> [source repository](https://github.com/JZ-05T68/engineering-knowledge-base-src) 公开；当前未提供独立安装包或打包后的可执行程序。

当前展示仓库为完整中文版。英文完整版自 v0.5.0 后提供；日文与韩文版自 v1.2.0 后提供；
俄文版自 v1.3.0 后提供。当前不提供半成品多语言入口。

| 项目状态 | 当前说明 |
| --- | --- |
| 当前阶段 | 已有本地可运行 MVP |
| 本次展示基线 | **v0.4.3** |
| 当前状态 | **v0.4.3 已正式发布**（RPV-01～11、H1～H7 与全量自动门禁通过） |
| 当前仓库 | 仅用于项目公开展示 |
| 源码与安装包 | 完整源码已公开；当前未提供独立安装包或打包后的可执行程序 |

> 公开发布摘要：[Engineering Knowledge Base v0.4.3](docs/v0.4.3-release-summary.md)
...
- RPV-01～11 与 H1～H7 全部 PASS；
- RPV-12 按 DF-AMEND-02 延后至 v0.5.0，不属于 v0.4.3 退出条件；
- OBS-E 仍为 MEDIUM 显式技术债，目标为 v0.5.x，未标记为已修复；
- v0.5.0 决策门为 CONDITIONAL GO，不表示 AI 集成已实现或验证；
- schema 保持 v7，无数据库迁移、无新依赖，本地优先与离线核心能力不变。
...
- v0.4.3 是当前稳定展示基线，已完成正式发布；
- v0.4.3 完成 RPV-01～11、H1～H7 和完整自动发布门禁；
- v0.2.0 的 335 项自动化测试、120 页自动化长文档基线和 300 页混合 PDF 规模验收保留为历史验证记录；
- 完整源码已在独立 source repository 公开；当前未提供独立安装包或打包后的可执行程序；
- 当前公开仓库继续作为项目展示页；
- 尚未形成规模化外部用户。
...
附件是 v0.1.2 时期的早期项目介绍，其中“尚未公开发布”只记录当时状态。当前完整源码已经公开，
尚未提供的是独立安装包或打包后的可执行程序；当前项目状态以本 README 和
[v0.4.3 公开发布摘要](docs/v0.4.3-release-summary.md) 为准。
